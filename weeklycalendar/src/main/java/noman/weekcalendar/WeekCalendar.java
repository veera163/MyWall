    ...
    private GridView getDaysNames() {
        ...
        daysName.setAdapter(new BaseAdapter() {
            ...
            private String[] getWeekDayNames() {
                ...
                daysName.remove(0);
                //daysName.add(daysName.remove(0)); -- Commented this in order to show Sunday at first position

                // Converting days names to upper case
                for (int i = 0; i < daysName.size(); i++)
                    daysName.set(i, daysName.get(i).toUpperCase());
